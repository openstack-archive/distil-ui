...
def _wash_details(current_details):
    ...
    dup_object_cost = (region_count - 1) * object_cost
...
def _get_quotations(request):
    ...
    # FIXME(flwang): Currently, Catalyst Cloud is still using v2 for auth so
    # the regions api doesn't work. And the existing AVAILABLE_REGIONS does
    # not fit this purpose. Will propose a separate to fix this.
    regions = ["nz-por-1", "nz-hlz-1", "nz_wlg_2"]
    try:
