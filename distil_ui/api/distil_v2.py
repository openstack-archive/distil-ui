...
def get_cost(request, distil_client=None):
    ...
    for i in range(len(ordered_invoices)):
        keys = list(ordered_invoices.keys())
        values = list(ordered_invoices.values())
        cost[i]["date"] = keys[i]
        cost[i]["total_cost"] = values[i]["total_cost"]
        cost[i]["status"] = values[i].get("status", None)
        parsed = _parse_invoice(values[i])
